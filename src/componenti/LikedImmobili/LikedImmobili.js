...
const LikedImmobili = (props) => {
    const [liked, setLiked] = useState(null);

    useEffect(() => {
        const fetchLiked = async () => {
          try {
            const risposta = await api.post("/api/v1/users/getting/user/liked/houses", {identificator: sessionStorage.getItem("identificator")});
            console.log("Immobili:", risposta.data);
            setLiked(risposta.data);
          } catch (errore) {
    ...
    if (!liked) {
        ...
        return (<SectionImmobili immobiliCards={liked}/>)
    }
}

export default LikedImmobili